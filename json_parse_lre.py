...
dataToTake = ["eventType", "eventName", "trackingEventNumber", "trackingFeaturedFaction", "trackingFeaturedHero", "maxStamina", "staminaRegenerationTime", "startStamina", "maxDailyStaminaRefreshAttempts", "rewardProgressId", "legendaryPassOfferId", "legendaryBundleOfferId", "heroEventMaxRuns", "finalEventShardsPerLegShard", "finalEventGoldPerLegCurrency", "finalEventLegShardsPerMythicShard", "minProgressionIndexToUseMythicConversion", "questId", "questGroups", "lanes", "ohSoCloseOffer"]
with open('lr_event.json', 'r') as file:
    lre_file = json.load(file)

lre_dat = lre_file["data"]
lre_waves = lre_file["waves"]
...
def formatLegendaryReleaseEvent():
    for event in lre_dat:
        ...
        eName = str(num) + " - " + unitName
        if lre_dat[event]["trackingEventNumber"] == 3:
            lreFormatted["Expired"][eName] = lre_dat[event]
        else:
            lreFormatted["Active"][eName] = {}
            for d in dataToTake:
                if d == "lanes":
                    waveName = event.replace("_hero", "")
                    lreFormatted["Active"][eName]["battles"] = {}
                    for lane in lre_dat[event]["lanes"]:
                        if lane["laneIndex"] == 1:
                            lreFormatted["Active"][eName]["battles"]["Alpha"] = lre_waves[waveName]["Alpha"]
                        if lane["laneIndex"] == 2:
                            lreFormatted["Active"][eName]["battles"]["Beta"] = lre_waves[waveName]["Beta"]
                        if lane["laneIndex"] == 3:
                            lreFormatted["Active"][eName]["battles"]["Gamma"] = lre_waves[waveName]["Gamma"]
                else:
                    lreFormatted["Active"][eName][d] = lre_dat[event][d]

    

formatLegendaryReleaseEvent()   
with open(fName, 'w') as w:
    json.dump(dict(sorted(lreFormatted.items())), w, indent=4)
lre_13 = {}
